...
export class CRMDashboard extends Component {
        ...
        /**
     * Handles the click event on Sales Orders card
     * Opens the Sales Order list view
     ...
     */
        sales_orders(e) {
            ...
            const filterValue = document.getElementById('income_expense_values').value;

            let domain = [['opportunity_id', '!=', false]];
    ...
    async onWillStart() {
      ...
      // In your dashboard widget
        // Get the filter value once at the start
        const selectedFilter = document.getElementById('income_expense_values')?.value || 'this_month';

        // Use this filter value consistently for all RPC calls
        var def14 = jsonrpc('/web/dataset/call_kw/crm.lead/get_sales_order_count', {
            model: "crm.lead",
            method: "get_sales_order_count",
            args: [[]],
            kwargs: {
                filter: selectedFilter,
            }
        }).then(function(res) {
            self.state.salesOrderCount = res['count'];
        });

        var def15 = jsonrpc('/web/dataset/call_kw/crm.lead/get_stage_counts', {
            model: "crm.lead",
            method: "get_stage_counts",
            args: [],
            kwargs: {
                date_filter: selectedFilter
            }
        }).then(function(result) {
            self.state.totalLeads = result.total_leads;
            self.state.advancedLeads = result.advanced_leads;
            self.state.ratio = result.ratio;
        });
