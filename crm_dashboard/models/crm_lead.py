...
class CRMLead(models.Model):
    ...
    total_leads = fields.Integer(compute="_compute_stage_counts")
    advanced_leads = fields.Integer(compute="_compute_stage_counts")
    total_advanced_ratio = fields.Float(compute="_compute_stage_counts")
    ...
    def _compute_stage_counts(self):
        for lead in self:
            ...
            # Get total leads
            lead.total_leads = self.env['crm.lead'].search_count([
                ('user_id', '=', self.env.user.id),
                ('type', '=', 'opportunity'),
                ('active', '=', True)
            ])

            # Get advanced stage leads (Prospect, Qualified, Demo, Proposal, Won, Lost)
            advanced_stages = stages.filtered(lambda s: any(word in s.name.lower()
                                                            for word in
                                                            ['prospect', 'qualified', 'demo', 'proposal', 'won',
                                                             'lost']))

            lead.advanced_leads = self.env['crm.lead'].search_count([
                ('user_id', '=', self.env.user.id),
                ('type', '=', 'opportunity'),
                ('active', '=', True),
                ('stage_id', 'in', advanced_stages.ids)
            ])

            # Calculate ratio
            if lead.advanced_leads > 0:
                lead.total_advanced_ratio = lead.total_leads / lead.advanced_leads
            else:
                lead.total_advanced_ratio = 0

    @api.model
    def get_stage_counts(self, date_filter=None):
        """Get total leads and advanced stage leads count for all users with date filter"""
        stages = self.env['crm.stage'].search([])
        ...
        if date_filter == 'this_week':
            ...
            domain += [('create_date', '>=', start), ('create_date', '<=', today)]
        ...
        # Get leads based on filtered domain
        leads = self.env['crm.lead'].search(domain)

        total_leads = len(leads)

        # Filter advanced stages based on your actual stage names
        advanced_stages = stages.filtered(lambda s: any(word in s.name.lower()
                                                        for word in
                                                        ['prospect', 'qualified', 'poc/demos/sow', 'proposal',
                                                         'negotiations', 'win', 'loss']))

        # Count leads in advanced stages
        advanced_leads = len(leads.filtered(lambda l: l.stage_id.id in advanced_stages.ids))

        # Calculate ratio
        ratio = advanced_leads and total_leads / advanced_leads or 0

        return {
            'total_leads': total_leads,
            'advanced_leads': advanced_leads,
            'ratio': round(ratio, 2)
        }


    def get_sales_order_count(self, filter=None):
