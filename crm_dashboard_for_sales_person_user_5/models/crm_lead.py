...
class CRMLead(models.Model):
    ...
    @api.model
    def _get_currency_sales_person_user(self):
    ...
    @api.model
    def check_user_group_sales_person_user(self, kwargs):
        """Checking a user group"""
        user = self.env.user
        if user.has_group('sales_team.group_sales_person_user'):
            return True
        else:
            return False

    @api.model
    def get_lead_stage_data_sales_person_user(self, kwargs):
        """funnel chart"""
    ...
    @api.model
    def get_lead_month_pie_sales_person_user(self, kwargs):
        """pie chart"""
        month_count = []
        month_value = []
    ...
    @api.model
    def get_the_sales_activity_sales_person_user(self, kwargs):
        """Sales Activity Pie"""
    ...
    @api.model
    def get_the_annual_target_sales_person_user(self, kwargs):
        ...
        if self.env.user.has_group('sales_team.group_sale_manager'):
            ...
            where stage_id=7 and team_id=%s AND Extract(Year FROM date_closed)=
    ...
    @api.model
    def get_the_campaign_pie_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_the_source_pie_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_the_medium_pie_sales_person_user(self, kwargs):
    ...
    @api.model
    def revenue_count_pie_sales_person_user(self, kwargs):
        """Total expected revenue and count Pie"""
        session_user_id = self.env.uid

        def fetch_total_revenue_sales_person_user(query):
        ...
        queries = [
            ...
            f"SELECT sum(expected_revenue) as revenue FROM crm_lead WHERE user_id={session_user_id} AND type='opportunity' AND active='false' AND stage_id='7'",
        ...
        total_expected_revenue, total_won_rev, total_lost_rev = [
            fetch_total_revenue_sales_person_user(query) for query in queries]
    ...
    @api.model
    def get_upcoming_events_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_top_deals_sales_person_user(self, kwargs):
        """Top 10 Deals Table"""
    ...
    @api.model
    def get_monthly_goal_sales_person_user(self, kwargs):
        """Monthly Goal Gauge"""
    ...
    @api.model
    def get_top_sp_revenue_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_country_revenue_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_country_count_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_total_lost_crm_sales_person_user(self, option):
    ...
    @api.model
    def get_ratio_based_country_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_ratio_based_sp_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_ratio_based_sales_team_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_lost_lead_by_reason_pie_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_lost_lead_by_stage_pie_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_recent_activities_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_count_unassigned_sales_person_user(self, kwargs):
    ...
    @api.model
    def get_top_sp_by_invoice_sales_person_user(self, kwargs):
    ...
    def lead_details_user(self, kwargs):
        ...
        self._cr.execute('''select sum(expected_revenue) from crm_lead where
         stage_id=7 AND Extract(Year FROM date_closed)=
        ...
        self._cr.execute('''select COUNT(id) from crm_lead WHERE stage_id=1 AND
        ...
        self._cr.execute('''select COUNT(id) from crm_lead WHERE stage_id=2 AND
        ...
        WHERE stage_id=3 AND type='opportunity' and active='true' 
        ...
        and stage_id=6 AND Extract(MONTH FROM crm_lead.date_closed) = 
        Extract(MONTH FROM DATE(NOW())) AND Extract(Year FROM 
        ...
        type='opportunity' and active = true and probability = 100 
    ...
    def crm_year(self, kwargs):
        ...
        opportunity_value = opportunity_data_value[0]
        self._cr.execute('''select SUM(crm_lead.expected_revenue) from crm_lead
        WHERE type='opportunity' and stage_id = 3 AND active='true' AND
        Extract(Year FROM crm_lead.date_deadline) = Extract(Year FROM 
        DATE(NOW()))''')
        exp_revenue_data = self._cr.dictfetchall()
        exp_revenue_data_value = [item['sum'] for item in exp_revenue_data]
        ...
        stage_id=6 AND Extract(Year FROM crm_lead.date_closed) = Extract(Year
        FROM DATE(NOW()))''')
        revenue_data = self._cr.dictfetchall()
        revenue_data_value = [item['sum'] for item in revenue_data]
        revenue_value = revenue_data_value[0]
        ...
        if revenue_value > 0:
            ...
            WHERE crm_lead.active = True AND 
            ...
            crm_lead.active = False AND crm_lead.probability 
        ...
        where type='opportunity' and active = true and probability = 100
        AND Extract(Year FROM date_closed) = Extract(Year 
    ...
    def crm_quarter(self, kwargs):
        ...
        session_user_id = self.env.uid
        self._cr.execute('''select COUNT(id) from crm_lead WHERE stage_id = 1 AND
         Extract(QUARTER FROM crm_lead.date_deadline)
        ...
        opportunity_value = opportunity_data_value[0]
        self._cr.execute('''select SUM(crm_lead.expected_revenue) from crm_lead 
        WHERE type='opportunity' AND stage_id = 3 AND active='true' 
        ...
        self._cr.execute('''select SUM(crm_lead.expected_revenue) from crm_lead 
        WHERE type='opportunity' and active='true' 
        and stage_id=6 AND Extract(QUARTER FROM crm_lead.date_closed) = 
        Extract(QUARTER FROM DATE(NOW())) AND Extract(Year FROM 
        crm_lead.date_closed) = Extract(Year FROM DATE(NOW()))''')
        revenue_data = self._cr.dictfetchall()
        revenue_data_value = [item['sum'] for item in revenue_data]
    ...
    def crm_month(self, kwargs):
        ...
        crm_lead WHERE type='opportunity' and 
        active='true' and stage_id=6 AND Extract(MONTH FROM 
        ...
        if revenue_value > 0:
            ...
            count_two from crm_lead WHERE 
            crm_lead.active = False AND crm_lead.probability = 0 
            AND Extract(MONTH FROM crm_lead.date_deadline) = 
            Extract(MONTH FROM DATE(NOW())) AND Extract(Year FROM 
            crm_lead.date_deadline) = Extract(Year FROM DATE(NOW())))b''')
            ratio_value = [row[0] for row in self._cr.fetchall()]
    ...
    def crm_week(self, kwargs):
        ...
        opportunity_value = opportunity_data_value[0]
        self._cr.execute('''select SUM(crm_lead.expected_revenue) from crm_lead 
        WHERE type='opportunity' AND stage_id = 3 
        ...
        if exp_revenue_value is None:
            exp_revenue_value = 0
        self._cr.execute('''select SUM(crm_lead.expected_revenue) from 
        crm_lead WHERE type='opportunity' 
        and active='true' and stage_id=6 AND Extract(
        ...
        self._cr.execute('''SELECT active,count(active) FROM crm_lead
        where type='opportunity' and active = true and probability = 100
        AND Extract(MONTH FROM crm_lead.date_closed) = 
        Extract(MONTH FROM DATE(NOW())) AND Extract(Week FROM 
    ...
    @api.model
    def _is_crm_sales_person_user(self):
        return self.env.user.has_group('crm_security.group_crm_sales_person_user_5')

    def write(self, vals):
        if self._is_crm_sales_person_user() and 'stage_id' in vals:
            allowed_stage_ids = self.env['crm.stage'].search([
                ('name', 'in', ['Suspect', 'Prospect'])
            ]).ids
            if vals['stage_id'] not in allowed_stage_ids:
                raise AccessError("You are only allowed to move leads between Suspect and Prospect.")
        return super(CRMLead, self).write(vals)


    @api.model
    def crm_half_year(self, kwargs):
        """Half-Year CRM Dropdown Filter (Jan–Jun or Jul–Dec based on current date)"""

        # crm_lead with stage_id = 1
        self._cr.execute("""
                         SELECT COUNT(id)
                         FROM crm_lead
                         WHERE stage_id = 1
                           AND CASE WHEN EXTRACT(MONTH FROM crm_lead.date_deadline) <= 6 THEN 1 ELSE 2 END =
                               CASE WHEN EXTRACT(MONTH FROM NOW()) <= 6 THEN 1 ELSE 2 END
                           AND EXTRACT(YEAR FROM crm_lead.date_deadline) = EXTRACT(YEAR FROM NOW())
                         """)
        crm_lead_value = self._cr.fetchone()[0] or 0
        ...
        self._cr.execute("""
                         ...
                         WHERE stage_id = 2
                           ...
                           AND EXTRACT(YEAR FROM crm_lead.date_deadline) = EXTRACT(YEAR FROM NOW())
                         """)
        opportunity_value = self._cr.fetchone()[0] or 0

        # Expected revenue (stage 3)
        self._cr.execute("""
                         ...
                         WHERE type = 'opportunity'
                           ...
                           AND EXTRACT(YEAR FROM crm_lead.date_deadline) = EXTRACT(YEAR FROM NOW())
                         """)
        exp_revenue_value = self._cr.fetchone()[0] or 0

        # Actual revenue (stage 6)
        self._cr.execute("""
                         SELECT SUM(expected_revenue)
                         FROM crm_lead
                         WHERE type = 'opportunity'
                           AND stage_id = 6
                           AND active = TRUE
                           AND CASE WHEN EXTRACT(MONTH FROM crm_lead.date_closed) <= 6 THEN 1 ELSE 2 END =
                               CASE WHEN EXTRACT(MONTH FROM NOW()) <= 6 THEN 1 ELSE 2 END
                           AND EXTRACT(YEAR FROM crm_lead.date_closed) = EXTRACT(YEAR FROM NOW())
                         """)
        revenue_value = self._cr.fetchone()[0] or 0

        # Win/Loss ratio
        self._cr.execute("""
                         SELECT CASE
                                    WHEN b.count_two = 0 THEN 0
                                    ELSE
                                        (CAST(a.count_one AS float) / CAST(b.count_two AS float)) END AS final_count
                         FROM (SELECT COUNT(id) AS count_one
                               FROM crm_lead
                               WHERE active = TRUE
                                 AND probability = 100
                                 AND CASE WHEN EXTRACT(MONTH FROM crm_lead.date_deadline) <= 6 THEN 1 ELSE 2 END =
                                     CASE WHEN EXTRACT(MONTH FROM NOW()) <= 6 THEN 1 ELSE 2 END
                                 AND EXTRACT(YEAR FROM crm_lead.date_deadline) = EXTRACT(YEAR FROM NOW())) a,
                              (SELECT COUNT(id) AS count_two
                               FROM crm_lead
                               WHERE active = FALSE
                                 AND probability = 0
                                 AND CASE WHEN EXTRACT(MONTH FROM crm_lead.date_deadline) <= 6 THEN 1 ELSE 2 END =
                                     CASE WHEN EXTRACT(MONTH FROM NOW()) <= 6 THEN 1 ELSE 2 END
                                 AND EXTRACT(YEAR FROM crm_lead.date_deadline) = EXTRACT(YEAR FROM NOW())) b
                         """)
        ratio_value = self._cr.fetchone()[0] or 0

        # Opportunity win/loss count ratio
        self._cr.execute("""
                         SELECT active, COUNT(active)
                         FROM crm_lead
                         WHERE type = 'opportunity'
                           AND (
                             (active = TRUE AND probability = 100 AND
                              CASE WHEN EXTRACT(MONTH FROM date_closed) <= 6 THEN 1 ELSE 2 END =
                              CASE WHEN EXTRACT(MONTH FROM NOW()) <= 6 THEN 1 ELSE 2 END
                                 AND EXTRACT(YEAR FROM date_closed) = EXTRACT(YEAR FROM NOW()))
                                 OR
                             (active = FALSE AND probability = 0 AND
                              CASE WHEN EXTRACT(MONTH FROM date_deadline) <= 6 THEN 1 ELSE 2 END =
                              CASE WHEN EXTRACT(MONTH FROM NOW()) <= 6 THEN 1 ELSE 2 END
                                 AND EXTRACT(YEAR FROM date_deadline) = EXTRACT(YEAR FROM NOW()))
                             )
                         GROUP BY active
                         """)
        record_opportunity = dict(self._cr.fetchall())
        total_won = record_opportunity.get(False, 0)
        total_lost = record_opportunity.get(True, 0)
        opportunity_ratio_value = round(total_won / total_lost, 2) if total_lost > 0 else 0.0

        # Average time to convert
        avg = 0
        if crm_lead_value > 0:
            self._cr.execute("""
                             SELECT date_conversion, create_date
                             FROM crm_lead
                             WHERE date_conversion IS NOT NULL
                               AND CASE WHEN EXTRACT(MONTH FROM create_date) <= 6 THEN 1 ELSE 2 END =
                                   CASE WHEN EXTRACT(MONTH FROM NOW()) <= 6 THEN 1 ELSE 2 END
                               AND EXTRACT(YEAR FROM create_date) = EXTRACT(YEAR FROM NOW())
                             """)
            records = self._cr.dictfetchall()
            for rec in records:
                avg += (rec["date_conversion"] - rec["create_date"]).total_seconds()
            record_avg_time = round(avg / crm_lead_value) if crm_lead_value else 0
        else:
            record_avg_time = 0
        ...
        self._cr.execute("""
                         ...
                         FROM sale_order sale
                                  JOIN crm_lead crm ON sale.opportunity_id = crm.id
                         WHERE CASE WHEN EXTRACT(MONTH FROM sale.date_order) <= 6 THEN 1 ELSE 2 END =
                               CASE WHEN EXTRACT(MONTH FROM NOW()) <= 6 THEN 1 ELSE 2 END
                           AND EXTRACT(YEAR FROM sale.date_order) = EXTRACT(YEAR FROM NOW())
                         """)
        sale_order_count = self._cr.fetchone()[0] or 0

        # Suspect to prospect
        self._cr.execute("""
                         SELECT COUNT(*) AS total,
                                COUNT(*)    FILTER (WHERE stage.name->>'en_US' NOT ILIKE '%Suspect%') AS prospect
                         FROM crm_lead lead
                                  JOIN crm_stage stage ON lead.stage_id = stage.id
                         WHERE lead.active = TRUE
                           AND lead.type = 'opportunity'
                           AND CASE WHEN EXTRACT(MONTH FROM lead.create_date) <= 6 THEN 1 ELSE 2 END =
                               CASE WHEN EXTRACT(MONTH FROM NOW()) <= 6 THEN 1 ELSE 2 END
                           AND EXTRACT(YEAR FROM lead.create_date) = EXTRACT(YEAR FROM NOW())
                         """)
        result = self._cr.fetchone()
        total_leads = result[0] or 0
        prospect_leads = result[1] or 0
        ...
        self._cr.execute("""
                         ...
                         WHERE lead.active = TRUE
                           AND lead.type = 'opportunity'
                           AND stage.name ->>'en_US' NOT ILIKE '%Suspect%'
                           AND
                             stage.name->>'en_US' NOT ILIKE '%Prospect%'
                           AND
                             CASE WHEN EXTRACT (MONTH FROM lead.create_date) <= 6 THEN 1 ELSE 2
                         END =
            CASE WHEN EXTRACT(MONTH FROM NOW()) <= 6 THEN 1 ELSE 2
                         END
            AND EXTRACT(YEAR FROM lead.create_date) = EXTRACT(YEAR FROM NOW())
                         """)
        opportunity_leads = self._cr.fetchone()[0] or 0
    ...
    def crm_year(self, kwargs):
        """Year CRM Dropdown Filter"""
        self._cr.execute(
            """select COUNT(id) from crm_lead WHERE stage_id = 1 AND
        Extract(Year FROM crm_lead.date_deadline) = 
        Extract(Year FROM DATE(NOW()))"""
        )
        record = self._cr.dictfetchall()
        ...
        crm_lead_value = rec_ids[0]
        self._cr.execute(
            """select COUNT(id) from crm_lead WHERE stage_id = 2 AND
        crm_lead.type = 'opportunity' AND 
        Extract(Year FROM crm_lead.date_deadline
        ) = Extract(Year FROM DATE(NOW()))"""
        )
        ...
        self._cr.execute(
            """
                         SELECT COUNT(*) AS sale_order_count FROM sale_order sale JOIN crm_lead crm ON sale.opportunity_id = crm.id 
                         WHERE sale.date_order >= date_trunc('year', CURRENT_DATE) AND sale.date_order < date_trunc('year', CURRENT_DATE) + INTERVAL '1 year';
                         """
        )
        sale_order_count = self._cr.fetchone()[0] or 0
        ...
        self._cr.execute(
            """
                         ...
                         WHERE lead.active = true
                           AND lead.type = 'opportunity'
                           AND lead.create_date >= date_trunc('year', CURRENT_DATE)
                           AND lead.create_date < date_trunc('year', CURRENT_DATE) + INTERVAL '1 year';
                         """
        )
        result = self._cr.fetchone()
        total_leads = result[0] or 0
        ...
        # -------prospect to opprtunity ratio
        self._cr.execute(
            """
                         SELECT COUNT(*) AS opportunity
                         FROM crm_lead lead
                                  JOIN crm_stage stage ON lead.stage_id = stage.id
                         WHERE lead.active = true
                           AND lead.type = 'opportunity'
                           AND stage.name ->>'en_US' NOT ILIKE '%Suspect%'
                           AND stage.name->>'en_US' NOT ILIKE '%Prospect%'
                           AND lead.create_date >= date_trunc('year'
                             , CURRENT_DATE)
                           AND lead.create_date
                             < date_trunc('year'
                             , CURRENT_DATE) + INTERVAL '1 year';
                         """
        )
        result = self._cr.fetchone()
        opportunity_leads = result[0] or 0
    ...
    @api.model
    def crm_all_history(self, kwargs):
        """All CRM History Data up to (today + 5 years)"""

        # Log the 5-year date range being used
        self._cr.execute("SELECT CURRENT_DATE + INTERVAL '5 years'")
        print(">>> [crm_all_history] Filter up to date:", self._cr.fetchone()[0])

        self._cr.execute("""
                         SELECT COUNT(id)
                         FROM crm_lead
                         WHERE stage_id = 1
                           AND date_deadline <= CURRENT_DATE + INTERVAL '5 years'
                         """)
        record = self._cr.dictfetchall()
        rec_ids = [item["count"] for item in record]
        crm_lead_value = rec_ids[0]
        ...
        self._cr.execute("""
                         ...
                         """)
        opportunity_data = self._cr.dictfetchall()
        opportunity_value = [item["count"] for item in opportunity_data][0]
        ...
        self._cr.execute("""
                         ...
                         """)
        exp_revenue_data = self._cr.dictfetchall()
        exp_revenue_value = exp_revenue_data[0]["sum"] or 0
        ...
        self._cr.execute("""
                         ...
                         WHERE type = 'opportunity'
                           AND stage_id = 6
                           AND active = TRUE
                           AND date_closed <= CURRENT_DATE + INTERVAL '5 years'
                         """)
        revenue_data = self._cr.dictfetchall()
        revenue_value = revenue_data[0]["sum"] or 0
        ...
        if revenue_value > 0:
            self._cr.execute("""
                             SELECT CASE
                                        WHEN b.count_two = 0 THEN 0
                                        ELSE (
                                            CAST(a.count_one AS float) / CAST(b.count_two AS float)
                                            ) END AS final_count
                             FROM (SELECT COUNT(id) AS count_one
                                   FROM crm_lead
                                   WHERE active = TRUE
                                     AND probability = 100
                                     AND date_deadline <= CURRENT_DATE + INTERVAL '5 years') a,
                                  (SELECT COUNT(id) AS count_two
                                   FROM crm_lead
                                   WHERE active = FALSE
                                     AND probability = 0
                                     AND date_deadline <= CURRENT_DATE + INTERVAL '5 years') b
                             """)
            ratio_value = [row[0] for row in self._cr.fetchall()]
            ratio_value = str(ratio_value)[1:-1]
        else:
        ...
        self._cr.execute("""
                         ...
                         WHERE type = 'opportunity'
                           AND (
                             (active = TRUE AND probability = 100 AND date_closed <= CURRENT_DATE + INTERVAL '5 years')
                                 OR
                             (active = FALSE AND probability = 0 AND date_deadline <= CURRENT_DATE + INTERVAL '5 years')
                             )
                         GROUP BY active
                         """)
        record_opportunity = dict(self._cr.fetchall())
        opportunity_ratio_value = 0.0
        total_opportunity_won = record_opportunity.get(False) or 0
        total_opportunity_lost = record_opportunity.get(True) or 0
        if total_opportunity_lost > 0:
        ...
        if crm_lead_value > 0:
            self._cr.execute("""
                             ...
                             FROM crm_lead
                             WHERE date_conversion IS NOT NULL
                               AND date_conversion <= CURRENT_DATE + INTERVAL '5 years'
                             """)
            data = self._cr.dictfetchall()
            for rec in data:
                date_close = rec["date_conversion"]
        ...
        self._cr.execute("""
                         ...
                         FROM sale_order sale
                                  JOIN crm_lead crm ON sale.opportunity_id = crm.id
                         WHERE sale.date_order <= CURRENT_DATE + INTERVAL '5 years'
                         """)
        sale_order_count = self._cr.fetchone()[0] or 0
        ...
        self._cr.execute("""
                         ...
                         WHERE lead.active = TRUE
                           AND lead.type = 'opportunity'
                           AND lead.create_date <= CURRENT_DATE + INTERVAL '5 years'
                         """)
        result = self._cr.fetchone()
        total_leads = result[0] or 0
        prospect_leads = result[1] or 0

        # Prospect to Opportunity Ratio
        self._cr.execute("""
                         SELECT COUNT(*) AS opportunity
                         FROM crm_lead lead
                                  JOIN crm_stage stage ON lead.stage_id = stage.id
